    ...
    actions: {
        ...
        async showSuccess({commit}, message) {
            ...
            if (message.status === 200) {
                await this.dispatch('reports/showMessage', message.data?.message || "Дані успішно оновлено");
        ...
        async showErrors({commit}, error) {
            ...
            if (error.response?.status === 422) {
                ...
                await this.dispatch('reports/showMessage', error.response?.data?.message || "Помилка при авторизації");
